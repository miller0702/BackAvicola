...
Customer.getCustomers = async () => {
    ...
    Customer_Balance AS (
        ...
        LEFT JOIN 
            Discounts_Sum D ON C.id = D.cliente_id
        GROUP BY 
            C.id, C.nombre, C.documento, C.telefono, SC.total_sales, PLA.ultimo_abono, D.total_discounts
    )
    SELECT
        CB.id, 
        CB.nombre, 
        ...
        CB.total_payments,
        CB.total_discounts,
        CB.total_sales - CB.total_payments - CB.total_discounts AS deuda_actual,
        CB.total_sales - (CB.total_payments - (CB.total_payments - CB.ultimo_abono)) - CB.total_discounts AS deuda_antigua
    FROM 
        Customer_Balance CB
    ORDER BY 
        CB.id;
